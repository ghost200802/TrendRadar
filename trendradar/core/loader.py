...
def _load_wechat_config(config_data: Dict) -> Dict:
    """加载微信公众号配置"""
    wechat = config_data.get("wechat", {})

    # 环境变量覆盖
    enabled_env = _get_env_bool("WECHAT_ENABLED")
    wewe_rss_url_env = _get_env_str("WECHAT_WEWE_RSS_URL")
    auth_code_env = _get_env_str("WECHAT_AUTH_CODE")

    # 解析 accounts 列表
    raw_accounts = wechat.get("accounts", [])
    accounts = []
    ...
    return {
        ...
        else wechat.get("enabled", False),
        "WEWE_RSS_URL": wewe_rss_url_env
        or wechat.get("wewe_rss_url", "http://localhost:4000"),
        "AUTH_CODE": auth_code_env or wechat.get("auth_code", ""),
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    # 微信公众号配置
    config["WECHAT"] = _load_wechat_config(config_data)
